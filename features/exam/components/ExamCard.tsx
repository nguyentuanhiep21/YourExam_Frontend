    ...
    <Link href={`/exam/${exam.id}`} className="block group min-w-[280px] w-[300px] sm:w-[320px] bg-white/90 backdrop-blur-sm rounded-[2rem] border border-slate-200/60 p-2 shadow-sm hover:shadow-[0_20px_40px_-15px_rgba(37,99,235,0.15)] hover:border-primary/40 hover:-translate-y-2 transition-all duration-500 cursor-pointer flex flex-col h-full snap-start overflow-hidden">
      ...
      <div className={`relative h-48 w-full rounded-[1.5rem] overflow-hidden mb-5 p-4 ${coverGradient}`}>
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
          <div className={`bg-white/90 backdrop-blur-md border border-white/80 rounded-xl px-4 py-2 shadow-sm text-xs font-bold tracking-wide ${tagTextColor} w-max`}>
            {exam.subject}
          </div>
          
          <div className="flex flex-col gap-1.5 mt-1">
            <div className="flex items-center gap-1.5 text-emerald-700 bg-white/80 backdrop-blur-md border border-white/60 px-2.5 py-1 rounded-lg w-max shadow-sm">
              <ThumbsUp className="w-3.5 h-3.5" />
              <span className="text-xs font-bold tracking-wide">{formatNumber(exam.upvotes)}</span>
            </div>

            <div className="flex items-center gap-1.5 text-blue-700 bg-white/80 backdrop-blur-md border border-white/60 px-2.5 py-1 rounded-lg w-max shadow-sm">
              <Download className="w-3.5 h-3.5" />
              <span className="text-xs font-bold tracking-wide">{formatNumber(exam.downloads)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 pb-4 mt-2 flex flex-col flex-1">
        ...
        <div className="flex-1">
          ...
          {exam.authorName && (
            <p className="text-sm text-slate-600 mt-2 font-medium font-body">
